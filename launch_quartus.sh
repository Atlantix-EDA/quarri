...
[ ! -f "$INJECT_LIB" ] && INJECT_LIB="$SCRIPT_DIR/crates/inject/target/release/libqss_inject.so"
...
if [ ! -f "$INJECT_LIB" ] || [ "$SCRIPT_DIR/crates/inject/src/lib.rs" -nt "$INJECT_LIB" ]; then
